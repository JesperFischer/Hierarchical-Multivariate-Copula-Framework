...
transformed parameters{
  ...
  for (n in 1:N){
    ...
    if(n == 1){
      ...
      alpha[n] = gm_alpha + drift * alpha[n-1] + fluc[n] * prec_alpha;
    }
    
    prob[n] = psycho(X[n], alpha[n], beta, lapse);
    entropy_t[n] = entropy(psycho(X[n], alpha[n], beta, lapse));
...
model {
  ...
  gm[13] ~ normal(0,1); //global mean of beta
  gm[14] ~ normal(0,1); //global mean of beta
  

  rt_ndt ~ normal(0.3,0.05);

  to_vector(fluc) ~ std_normal();


  for (n in 1:N) {


    target += bernoulli_lpmf(binom_y[n] | prob[n]);
...
generated quantities{
  ...
  real threshold = gm_alpha / (1 - drift);
