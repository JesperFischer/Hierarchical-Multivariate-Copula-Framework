...
transformed parameters{
  ...
  row_vector[S] alpha = exp(param[1,]);
  row_vector[S] beta = inv_logit(param[2,]);
  row_vector[S] delta = (param[3,]);
  row_vector[S] delta_x = (param[4,]);
...
model {
  ...
  for(n in 1:N){
    int c = binom_y[n];
    if(c == 1){
      RT[n] ~ wiener(alpha[S_id[n]],rt_ndt[S_id[n]],beta[S_id[n]],x[n]*delta[S_id[n]]);
    }else if(c == 0){
      RT[n] ~ wiener(alpha[S_id[n]],rt_ndt[S_id[n]],1-beta[S_id[n]],-x[n]*delta[S_id[n]]);
    }
  }
...
generated quantities {
  ...
  real mu_beta = gm[2];
  real mu_delta = gm[3];
  real mu_delta_x = gm[4];

  real tau_alpha = tau_u[1];
  real tau_beta = tau_u[2];
  real tau_delta = tau_u[3];
  real tau_delta_x = tau_u[4];
  ...
  for(n in 1:N){
    int c = binom_y[n];
    if(c == 1){
      log_lik[n] = wiener_lpdf(RT[n] | alpha[S_id[n]],rt_ndt[S_id[n]],beta[S_id[n]],x[n]*delta[S_id[n]]);
    }else if(c == 0){
      log_lik[n] = wiener_lpdf(RT[n] | alpha[S_id[n]],rt_ndt[S_id[n]],1-beta[S_id[n]],-x[n]*delta[S_id[n]]);
    }
  }
