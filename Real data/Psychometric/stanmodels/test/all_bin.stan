functions {
  ...
  matrix uvar_bounds_conf(array[] int binom_y, vector ACC, vector gm, vector tau_u,matrix L_u, matrix z_expo, array[] int S_id, vector X,
                     int is_upper) {
    int N = size(binom_y);
  ...
  vector[S] conf_entropy_ACC = param[,11];
    ...
    for (n in 1:N) {
      ...
      real entropy_t = entropy(psycho(X[n], alpha[S_id[n]], beta[S_id[n]], lapse[S_id[n]]));
      ...
      if (is_upper == 0) {
        u_bounds[n, 1] = binom_y[n] == 0.0
                          ? 0.0 : binomial_cdf(binom_y[n] - 1 | 1, inv_logit(conf_mu));
      } else {
        u_bounds[n, 1] = binomial_cdf(binom_y[n] | 1, inv_logit(conf_mu));
      }
    }
...
parameters {
  ...
  >[N, 1] u;
    ...
    matrix<
    lower=uvar_bounds_conf(conf_y,ACC, gm, tau_u,L_u,z_expo, S_id,X, 0),
    upper=uvar_bounds_conf(conf_y,ACC, gm, tau_u,L_u,z_expo, S_id,X, 1)
  >[N, 1] u_conf;
...
model {
  ...
  gm[8:11] ~ normal(0,1); //global mean of beta
